    ...
    async def _acting(self, tool_call: ToolUseBlock) -> Msg | None:
        ...
        try:
            ...
            async for chunk in tool_res:
                ...
                # 检查 chunk.metadata 是否为字典类型且不为 None
                if chunk.metadata is not None and isinstance(chunk.metadata, dict) and chunk.metadata.get(
                    "update_memory",
                ):
                    update_memory = True
            ...
            if tool_call["name"] == self.search_function:
                ...
                # 检查 extract_res.metadata 是否为字典类型且不为 None
                if extract_res.metadata is not None and isinstance(
